  ...
  return (
    <div className="mx-auto w-full max-w-6xl px-4 py-6 sm:px-6 space-y-6">
      ...
      <div className="flex flex-wrap items-center justify-between gap-3">
        <div>
          <p className="font-mono text-[11px] uppercase tracking-[0.15em] text-amber-300 font-bold">
            MVP Demo · Pilot toolkit · Mock data
          </p>
          <h1 className="mt-1 font-serif text-3xl sm:text-4xl text-white">
            Pilot board <span className="italic text-white/70">for the clinic visit.</span>
          </h1>
        ...
        <div className="flex gap-2">
          <Link href="/console" className="rounded-full bg-white px-4 py-2.5 text-xs font-bold text-black hover:bg-white/85 min-h-[44px] flex items-center">
            OT Console
          </Link>
          <Link href="/" className="rounded-full border border-white/20 bg-white/10 px-4 py-2.5 text-xs text-white hover:bg-white/20 min-h-[44px] flex items-center">
            Home
          </Link>
        </div>
      </div>
      ...
      <section aria-label="Pilot metrics" className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        {[
          ...
          <div key={c.label} className="rounded-2xl border border-white/15 bg-white/10 p-4 backdrop-blur-md">
            <p className="font-mono text-[10px] uppercase tracking-wider text-white/50">{c.label}</p>
            <p className="mt-1 font-serif text-3xl text-white">{c.value}</p>
      ...
      {metrics.overloadedCases > 0 && (
        <div role="note" className="rounded-xl border border-amber-400/40 bg-amber-400/10 p-3 text-xs text-amber-200">
      ...
      {/* Baseline editor */}
      <section aria-label="Baseline editor" className="rounded-2xl border border-white/15 bg-white/10 p-4 sm:p-5 backdrop-blur-md">
        <h2 className="font-serif text-xl text-white">1 · Baseline <span className="italic text-white/60">(fill in with your dad before the visit)</span></h2>
          ...
          {(
            ...
            <label key={key} className="block">
              ...
              <input
                ...
                className="mt-1 w-full rounded-lg border border-white/20 bg-black/40 px-2 py-2 text-xs text-white focus:outline-none focus:border-sky-300"
              />
            </label>
          ))}
        </div>
        <button
          ...
          className="mt-3 rounded-xl bg-white px-4 py-2 text-xs font-bold text-black hover:bg-white/85 min-h-[44px]"
        >
      ...
      {/* Call queue */}
      <section aria-label="Coordinator call queue" className="rounded-2xl border border-white/15 bg-white/10 p-4 sm:p-5 backdrop-blur-md">
        <h2 className="font-serif text-xl text-white">2 · Coordinator call queue <span className="italic text-white/60">(highest risk first)</span></h2>
        <p className="mt-1 text-xs text-white/60">
        ...
        <div className="mt-3 divide-y divide-white/10">
            ...
            return (
              <div key={q.patient.id} className="py-3">
                <button
                  ...
                  aria-expanded={open}
                >
                  <span className="font-mono text-xs text-white/40 w-6 shrink-0">{i + 1}</span>
                  <span className="min-w-0 flex-1">
                    <span className="block truncate text-sm font-semibold text-white">
                      {q.patient.name} <span className="font-mono text-[10px] text-white/50">{q.patient.mrn}</span>
                    </span>
                  ...
                  {q.outOfScope.length > 0 && (
                    <span className="shrink-0 rounded-full border border-fuchsia-400/40 bg-fuchsia-500/10 px-2 py-0.5 font-mono text-[10px] font-bold text-fuchsia-200">
                      OUT OF SCOPE
                    </span>
                  )}
                  {done && (
                    <span className="shrink-0 rounded-full border border-emerald-400/40 bg-emerald-500/10 px-2 py-0.5 font-mono text-[10px] font-bold text-emerald-200">
                      ✓ DAY-BEFORE DONE
                    </span>
                  )}
                </button>
                {open && (
                  <div className="mt-2 ml-9 space-y-2 rounded-xl border border-white/10 bg-black/30 p-3">
                    {q.outOfScope.length > 0 && (
                      <p className="text-[11px] text-fuchsia-200">Scope: {q.outOfScope.join(' · ')}</p>
                    )}
                    <ul className="space-y-1">
                      {q.script.map((s, k) => (
                        <li key={k} className="text-xs text-white/75">• {s}</li>
                      ))}
                    </ul>
                    <div className="flex flex-wrap gap-2 pt-1">
                      ...
                      <button
                        ...
                        className="rounded-lg border border-sky-400/40 bg-sky-500/15 px-3 py-2 text-[11px] font-bold text-sky-200 min-h-[44px]"
                      >
          ...
          {queue.length === 0 && <p className="py-4 text-xs text-white/50">No mock cases loaded.</p>}
        </div>
      </section>

      {/* OT schedule cross-check */}
      <section aria-label="OT schedule cross-check" className="rounded-2xl border border-white/15 bg-white/10 p-4 sm:p-5 backdrop-blur-md">
        <h2 className="font-serif text-xl text-white">3 · OT-list cross-check <span className="italic text-white/60">(paste the day&apos;s list)</span></h2>
        <p className="mt-1 text-xs text-white/60">
        ...
        <textarea
          ...
          className="mt-3 w-full rounded-xl border border-white/20 bg-black/40 p-3 font-mono text-xs text-white focus:outline-none focus:border-sky-300"
        />
        <button
          type="button"
          onClick={runCsv}
          className="mt-2 rounded-xl bg-white px-4 py-2 text-xs font-bold text-black hover:bg-white/85 min-h-[44px]"
        >
        ...
        {check && (
          <div className="mt-3 grid gap-3 sm:grid-cols-2">
            <div className="rounded-xl border border-emerald-400/30 bg-emerald-500/10 p-3">
              <p className="font-mono text-[11px] font-bold text-emerald-200">MATCHED · {check.matched.length}</p>
              {check.matched.map((r) => (
                <p key={r.mrn} className="mt-1 text-xs text-white/75">{r.mrn} — {r.name}</p>
              ))}
              {check.matched.length === 0 && <p className="mt-1 text-xs text-white/50">None.</p>}
            </div>
            <div className="rounded-xl border border-rose-400/30 bg-rose-500/10 p-3">
              <p className="font-mono text-[11px] font-bold text-rose-200">NO DEMO FILE · {check.unmatched.length}</p>
              {check.unmatched.map((r, k) => (
                <p key={`${r.mrn}-${k}`} className="mt-1 text-xs text-white/75">{r.mrn || '(no MRN)'} — {r.name || r.procedure}</p>
              ))}
              {check.unmatched.length === 0 && <p className="mt-1 text-xs text-white/50">None — full coverage.</p>}
            </div>
          </div>
        )}
      </section>
      ...
      <section aria-label="PureLab inbox" className="rounded-2xl border border-white/15 bg-white/10 p-4 sm:p-5 backdrop-blur-md">
        <h2 className="font-serif text-xl text-white">5 · PureLab inbox <span className="italic text-white/60">(email → PAC, synthetic demo)</span></h2>
        <p className="mt-1 text-xs text-white/60">
      ...
      <section aria-label="Pilot scope and liability" className="rounded-2xl border border-amber-400/40 bg-amber-400/10 p-4 sm:p-5">
        <h2 className="font-serif text-xl text-amber-100">4 · Scope &amp; liability boundary <span className="italic">(read aloud at the visit)</span></h2>
