...
export const ProvenanceInspector: React.FC<ProvenanceInspectorProps> = ({
  ...
  return (
    <div
      className="flex h-full flex-col rounded-[6px] border border-black/10 bg-white shadow-none overflow-hidden"
      role="complementary"
      aria-label="OCR Provenance Inspector"
    >
      {/* Header Controls */}
      <div className="flex flex-wrap items-center justify-between border-b border-black/10 bg-white px-4 py-3 gap-2">
        <div className="flex items-center gap-2.5">
          <Scan className="h-4 w-4 text-[#1a1a1a]" aria-hidden="true" />
          <div>
            <div className="flex items-center gap-2">
              <h3 className="font-serif text-sm font-bold text-[#1a1a1a] flex items-center gap-1.5">
                {t('provenance.title')}
              </h3>
              <span className="rounded-[3px] bg-white px-1.5 py-0.2 font-mono text-[9.5px] font-bold text-black border border-black/25">
                VERIFIED SOURCE RECORD
              </span>
            </div>
            <p className="text-[10.5px] text-[#6b706b] font-mono truncate max-w-xs sm:max-w-md">
        ...
        <div className="flex items-center gap-2 text-xs font-mono">
          {/* Page Switcher */}
          <div className="flex items-center rounded-[4px] border border-black/10 bg-white p-0.5">
            <button
              type="button"
              onClick={() => setActivePage((p) => Math.max(1, p - 1))}
              disabled={activePage <= 1}
              className="p-1 text-[#6b706b] hover:text-[#1a1a1a] disabled:opacity-30 cursor-pointer"
            >
              <ChevronLeft className="h-3.5 w-3.5" />
            </button>
            <span className="px-2 text-[11px] text-[#1a1a1a] font-semibold">
          ...
          {/* Zoom Controls */}
          <div className="flex items-center gap-1 rounded-[4px] border border-black/10 bg-white p-0.5">
            <button
              type="button"
              onClick={() => setZoomLevel((prev) => Math.max(70, prev - 15))}
              className="p-1 text-[#6b706b] hover:text-[#1a1a1a] rounded-[3px] cursor-pointer"
              title={t('provenance.zoomOut')}
            >
              <ZoomOut className="h-3.5 w-3.5" />
            </button>
            <span className="px-1 text-[10.5px] text-[#1a1a1a]">{zoomLevel}%</span>
            <button
              type="button"
          ...
          {onClose && (
            <button
              type="button"
              onClick={onClose}
              className="rounded-[4px] border border-black/10 bg-white px-2.5 py-1 text-xs text-[#1a1a1a] hover:text-[#1a1a1a] hover:border-black/25 cursor-pointer"
            >
              {t('provenance.dock')}
            </button>
          )}
      ...
      <div className="grid flex-1 grid-cols-1 overflow-hidden lg:grid-cols-12">
        ...
        <div className="relative overflow-auto bg-white p-4 lg:col-span-7 flex items-center justify-center min-h-[420px]">
          {uploadedPdfDataUrl ? (
            <div
              style={{ transform: `scale(${zoomLevel / 100})`, transformOrigin: 'top center' }}
              className="relative shadow-2xl transition-transform duration-150"
            >
              <canvas ref={canvasRef} className="rounded-[4px] max-w-full bg-white block" />
            ...
            <div
              style={{ transform: `scale(${zoomLevel / 100})`, transformOrigin: 'top center' }}
              className="w-full max-w-[620px] min-h-[640px] rounded-[4px] border border-black/10 bg-[#FAFAFA] text-[#1a1a1a] p-6 relative select-none font-sans text-xs transition-transform duration-150 shadow-2xl"
            >
              <div
                className="pointer-events-none absolute inset-0 flex items-center justify-center opacity-5 select-none font-serif text-5xl font-bold text-[#1a1a1a] rotate-[-30deg]"
                aria-hidden="true"
              >
              ...
              <div className="border-b-2 border-slate-900 pb-3 flex justify-between items-start">
                <div>
                  <h4 className="font-serif font-black text-xs uppercase tracking-tight text-[#1a1a1a]">
                    {docTitle}
                  </h4>
                  <p className="text-[9.5px] text-[#6b706b] font-sans mt-0.5">{docSubtitle}</p>
                  <p className="text-[9px] text-[#6b706b] font-mono mt-0.5">
                    Specimen Collected: {new Date().toLocaleDateString()} 07:15 GST · Verified by Chief Pathologist
                  </p>
                </div>
                <div className="text-right font-mono text-[9.5px] text-[#6b706b]">
                  <div className="font-bold text-[#1a1a1a]">PATIENT CASE FILE</div>
              ...
              <div className="mt-4 space-y-1 relative">
                <div className="grid grid-cols-12 text-[10px] font-mono font-bold text-[#3f4440] border-b border-black/25 pb-1.5 mb-2">
                ...
                {patient.labs.map((lab) => {
                  ...
                  return (
                    ...
                    >
                      <div className="col-span-5 font-medium text-[#1a1a1a] flex items-center gap-1.5 truncate">
                      ...
                      <div className="col-span-2 text-center text-[10px] text-[#6b706b]">{lab.unit}</div>
                      <div className="col-span-3 text-right text-[10px] text-[#6b706b]">
              ...
              {/* Watermark Footer */}
              <div className="absolute bottom-3 left-6 right-6 pt-2 border-t border-black/25 flex justify-between text-[9px] font-mono text-[#6b706b]">
        ...
        <div className="border-t lg:border-t-0 lg:border-l border-black/10 bg-white p-4 lg:col-span-5 space-y-4 overflow-y-auto">
          <div>
              ...
              <span
                className={`rounded px-1.5 py-0.2 font-mono text-[9.5px] font-bold ${
                  selectedLab?.status === 'NORMAL'
                    ? 'bg-white text-[#1a1a1a] border border-black/25'
            ...
            {selectedLab ? (
              <div className="mt-3 rounded-[5px] border border-black/10 bg-white p-3 space-y-2.5">
                <div className="flex items-baseline justify-between">
                  <h4 className="font-sans text-sm font-bold text-[#1a1a1a]">{selectedLab.name}</h4>
                  <div className="font-mono text-base font-bold text-[#1a1a1a]">
                    {selectedLab.value}{' '}
                    <span className="text-xs text-[#6b706b] font-normal">{selectedLab.unit}</span>
                  </div>
                </div>

                <div className="grid grid-cols-2 gap-2 text-[10.5px] font-mono border-t border-black/10 pt-2">
                  <div>
                    <span className="text-[#6b706b] block">LOINC CODE</span>
                    <span className="font-bold text-[#1a1a1a]">{selectedLab.loinc}</span>
                  </div>
                  <div>
                    <span className="text-[#6b706b] block">REF INTERVAL</span>
                ...
                <div className="border-t border-black/10 pt-2">
                  <span className="text-[10px] font-mono text-[#6b706b] uppercase block mb-0.5">
                    CLINICAL DIRECTIVE
                  </span>
                  <p className="text-xs font-sans text-[#1a1a1a] leading-relaxed font-medium">
                    {selectedLab.directive}
                  </p>
                </div>
              </div>
            ) : (
              <div className="mt-3 rounded-[5px] border border-black/10 bg-white p-4 text-center text-[#6b706b] text-xs">
          ...
          {selectedLab && (
            <div className="rounded-[5px] border border-black/10 bg-white p-3 space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-[10.5px] font-mono text-[#6b706b] uppercase tracking-wider flex items-center gap-1.5">
                  <Hash className="h-3 w-3 text-[#1a1a1a]" />
                  <span>Raw OCR String</span>
                </span>
                <button
                  type="button"
                  onClick={() => copyToClipboard(selectedLab.provenance.rawOcrText)}
                  className="flex items-center gap-1 text-[10px] font-mono text-[#1a1a1a] hover:text-[#6b706b] cursor-pointer"
                >
              ...
              <div className="rounded-[3px] bg-black/[0.03] p-2 font-mono text-[11px] text-[#3f4440] border border-black/10 break-all">
                &gt; {selectedLab.provenance.rawOcrText}
              </div>

              <div className="grid grid-cols-2 gap-2 text-[10px] font-mono text-[#6b706b] pt-1">
                <div>Confidence: <span className="text-[#1a1a1a] font-bold">{Math.round(selectedLab.provenance.confidence * 100)}%</span></div>
                <div>Source Page: <span className="text-[#1a1a1a] font-bold">{selectedLab.provenance.page}</span></div>
              </div>
            </div>
          )}
          ...
          <div>
            <div className="text-[10.5px] font-mono text-[#6b706b] uppercase tracking-wider mb-1.5">
                ...
                <button
                  ...
                  className={`w-full flex items-center justify-between p-2 rounded-[3px] text-xs font-mono transition text-left cursor-pointer ${
                    selectedLab?.id === lab.id
                      ...
                      : 'border border-black/10 bg-white text-[#3f4440] hover:border-black/20'
                  }`}
                >
